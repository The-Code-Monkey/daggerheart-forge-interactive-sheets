...
const ClassDomains = ({ form }: ClassDomainsProps): JSX.Element => {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-2xl font-semibold text-white mb-4">Available Domains</h3>
      <p className="text-purple-200 mb-4">
        Select the domains that this class can access. Classes typically have access to 2-3 domains.
      </p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {domains.map((domain) => (
          <div key={domain.id} className="bg-slate-800/30 p-4 rounded-lg border border-brand-500/20">
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                {domain.description && (
                  <p className="text-purple-200 text-sm mt-1">{domain.description}</p>
                )}
              </div>
            </div>
          </div>
